      ...
      <div className="bg-white p-8 rounded-lg shadow-lg text-center">
        <div className="flex justify-center items-center mb-4">
            ...
            <div className="ml-4 text-left">
                ...
                <h2 className="text-3xl font-bold text-warm-purple-700">{currentLevel.name}</h2>
            </div>
        </div>
        ...
        {nextLevel ? (
            <p className="mt-4 text-gray-500">
                You need <span className="font-bold text-warm-purple-600">{nextLevel.minPoints - userPoints}</span> more points to reach {nextLevel.name}.
            </p>
        ) : (
       ...
       <div className="bg-white p-6 rounded-lg shadow-md">
            <h3 className="text-xl font-semibold text-gray-700 mb-4">All Levels</h3>
            <ul className="space-y-3">
                {levels.map((level, index) => (
                    <li key={level.name} className={`flex items-center p-3 rounded-lg ${index === currentLevelIndex ? 'bg-warm-purple-100 ring-2 ring-warm-purple-300' : 'bg-gray-50'}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-4 ${index <= currentLevelIndex ? 'bg-warm-purple-500 text-white' : 'bg-gray-300 text-gray-600'}`}>
                            <Star className="w-5 h-5" />
                        </div>
                        <div>
                            <p className="font-semibold text-gray-800">{level.name}</p>
                            <p className="text-sm text-gray-500">Requires {level.minPoints} points</p>
                        </div>
                    </li>
                ))}
            </ul>
